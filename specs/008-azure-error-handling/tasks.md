...
- [X] T020 [US4] Update existing tests that return empty results to account for new `ErrNotFound` behavior: `TestClient_GetPrices_UserAgent` and `TestClient_GetPrices_Logging` in `internal/azureclient/client_test.go` must provide non-empty responses or assert `ErrNotFound`
...
- **US1 and US2 can run in parallel** (US1 modifies `GetPrices`,
  US2 modifies `fetchPage` + creates new files).
  **Note**: Both US1 and US2 add tests to `client_test.go`,
  which can cause merge conflicts. Contributors should coordinate
  by sequencing test-related commits, using short-lived feature
  branches, rebasing/merging before pushing, or claiming the test
  file for a change window.
- **T012 and T013 can run in parallel** (different files:
  client_test.go vs errors_test.go)
- **T030, T031, T032, T033, T034 can all run in parallel**
  (different concerns). **Note**: T030–T034 may touch shared
  test infrastructure; coordinate as above if multiple
  contributors work concurrently.

---

## Parallel Example: US1 + US2

> **Shared-file warning**: Both US1 (T005–T007) and US2 (T012) write to
> `client_test.go`. To avoid merge conflicts, contributors should sequence
> test-related commits, use short-lived feature branches, rebase/merge
> before pushing, or claim the test file for a change window.

```text
# These two story phases can execute simultaneously:

# Developer A: US1 (GetPrices context wrapping)
T005: Test error includes query context     -> client_test.go  (⚠ shared)
T006: Test error preserves root cause       -> client_test.go  (⚠ shared)
T007: Test mid-pagination error             -> client_test.go  (⚠ shared)
...
T012: Test HTTP 404 -> ErrNotFound          -> client_test.go  (⚠ shared)
